...
if uploaded_file is not None:
    ...
    # Random forest model
    models = ["None", "Random Forest Regression"]
    ...
    if selected_cols and plot_type and model_selection != "None":
        ...
        if model_selection == "Random Forest Regression":
            ...
            fig = px.scatter(df, x=selected_cols[0], y=selected_cols[1],
                             title=f"Scatter Plot with Random Forest Regression: {selected_cols[0]} vs {selected_cols[1]}",
                             labels={selected_cols[0]: selected_cols[0], selected_cols[1]: selected_cols[1]})
            
            # Add random forest regression line
            fig.add_scatter(x=X_pred.flatten(), y=y_pred, mode='lines', name='Random Forest Regression')
