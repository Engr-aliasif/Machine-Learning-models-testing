...
if uploaded_file is not None:
    ...
    models = ["None", "K-Nearest Neighbors Regression"]
    ...
    if selected_cols and plot_type and model_selection != "None":
        ...
        if model_selection == "K-Nearest Neighbors Regression":
            ...
            fig = px.scatter(df, x=selected_cols[0], y=selected_cols[1],
                             title=f"Scatter Plot with K-Nearest Neighbors Regression: {selected_cols[0]} vs {selected_cols[1]}",
                             labels={selected_cols[0]: selected_cols[0], selected_cols[1]: selected_cols[1]})
            
            # Add K-Nearest Neighbors regression line
            fig.add_scatter(x=X_pred.flatten(), y=y_pred, mode='lines', name='KNN Regression')
